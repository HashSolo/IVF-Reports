...
namespace :db do
  desc "Add the ranking data for 2008."
  task :calc_ranking_data_2008 => :environment do
    ...
    #The national_average will be identified by a clinic_id of 9999
	  ...
	  year_index = 3 #Housekeeping for the year loop, but we should do a separate file for each year (moving forward we would only do one year at a time)
	  while(year_index < 4)
