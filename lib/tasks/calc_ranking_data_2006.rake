...
namespace :db do
  desc "Add the ranking data for 2006."
  task :calc_ranking_data_2006 => :environment do
    ...
    #The national_average will be identified by a clinic_id of 9999
	  ...
	  year_index = 1 #Housekeeping for the year loop, but we should do a separate file for each year (moving forward we would only do one year at a time)
	  while(year_index < 2)
	    ...
	    while(diagnosis_index < 11)
          ...
          clinics.each do |c|
            ...
            unless cur_datapoint.empty?
              #We only want to include clinics with >= 10 cycles in the rankings (for individual age groups) AND WE DON'T WANT THE NATIONAL DATA
          ...
          while(score_calc_index < array_index)
            ...
            puts "Year: #{years[year_index]} Age Group: #{age_groups[age_group_index]} Diagnosis: #{diagnoses[diagnosis_index]}  | Quality Score: #{imp_final_scores[score_calc_index]}."
